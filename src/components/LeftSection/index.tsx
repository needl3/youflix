...
export default function LeftSection() {
    ...
    const [followers, setFollowers] = useState(
        dummyFollowers.slice(0, DEFAULT_SHOW)
    )
    ...
    function fetchMoreFollowers() {
        ...
        //
        setTimeout(() => {
            if (followers.length <= DEFAULT_SHOW) setFollowers(dummyFollowers)
        }, 2000)
    }
        ...
        <ul className={`py-3 ${active ? 'px-3' : ''}`}>
            {mainMenu.map(m => (
                <Link href={m.href} key={m.name}>
                    ...
                    ${
                        active
                            ? 'text-sm pl-3 flex-row pr-16 py-2 gap-x-6'
                            : 'text-[10px] flex-col p-1 py-4'
                    }
            ...
            {active ? (
                <>
                    {followers
                        .slice(0, followShowAll ? -1 : DEFAULT_SHOW)
                        .map(f => (
                            <Link href={`profile/${f.href}`} key={f.name}>
                    ...
                    <li
                        className="flex rounded-lg hover: cursor-pointer hover:bg-slate-400 text-sm items-center py-2 gap-x-6 pl-3 pr-14"
                        onClick={() => {
                            fetchMoreFollowers()

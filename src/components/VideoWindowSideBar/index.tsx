...
export default function VideoWindowSidebar({ fetchMore, info, videos }: { fetchMore: fetchMoreVideosFunctionType, info: Prisma.MovieCreateInput, videos: MovieDetail[] }) {
    ...
    return <>
        ...
        <ul>
            {paginatingVideos.map(video => <Link href={`${video.id}`} key={video.id}>
                <li className="py-1 grid grid-cols-12">
                    <div className="relative flex-grow col-span-5">
                        <img src={video.image || "images/placeholder.png"} className="rounded-lg h-24 w-full" referrerPolicy="no-referrer" />
                        <span className="absolute bottom-1 right-1 bg-white rounded-md px-1 text-sm">{parseLength(/* item.length */0 * 60 * 60 * 1000)}</span>
                    </div>
                    <div className="col-span-7 pl-2">
                        <h2 className="text-sm font-bold">{video.name}</h2>

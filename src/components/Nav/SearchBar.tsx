...
export default function SearchBar() {
    ...
    function searchHandler(
        ...
        if (e.key == 'Enter') console.log('Haha')
    ...
    return (
        <div className="flex w-[40rem] gap-x-4 h-10 items-center text-center">
            <div className="items-center flex h-full w-full">
                <div
                    className="border rounded-l-full border-[#aaaaaa] pl-4 bg-white h-full w-full"
                ...
                >
                    <input
                        ...
                        name="query-bar"
                        onChange={onChange}
                        className="focus:outline-none text-lg h-full w-full"
                    />
                </div>
                <button
                    onClick={searchHandler}
                    className="px-2 border border-l-0 border-[#aaaaaa] rounded-r-full h-full flex"
                >
                    <Search />
                </button>
            </div>
            <button
                onClick={searchHandler}
                className="p-2 rounded-full h-full bg-slate-200"
            >
                <Mic />
            </button>
        </div>

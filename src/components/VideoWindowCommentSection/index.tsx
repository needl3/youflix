    ...
    return (
        <>
            <div className="flex gap-x-5 pb-5">
                <p className="text-xl">
                    {paginatingComments.length + ' Comments'}
                </p>{' '}
                {/* Remove this 432 text*/}
                <button className="font-bold flex gap-x-2">
                    <Filter /> Sort By
                </button>
            </div>
            <AddCommentComponent
            ...
            />
            <ul>
                {paginatingComments.map(comment => (
                    <li key={comment.id}>
                        <CommentItem
                            disableSubComments={false}
                            createComment={createComment}

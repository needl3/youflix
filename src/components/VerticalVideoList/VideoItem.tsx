...
export function parseViews(views: number): string {
    ...
    if (sizeNum >= 4) toReturn = String(views / 1000) + 'K'
    else if (sizeNum >= 7) toReturn = String(views / 1000000) + 'M'
    else if (sizeNum >= 10) toReturn = String(views / 1000000000) + 'B'
...
export function parsePublishedDate(d: Date): string {
    ...
    //
    if (d.getTime() !== d.getTime()) return 'X'
    ...
    else parsedTime = String(Math.floor(secondsElasped / oneSec)) + ' seconds'
...
export function parseLength(ms: number) {
    ...
    if (ms > oneHour) parsedLength += String(hr) + ':'
...
export default function VideoItem({
    ...
    item: Prisma.MovieCreateManyInput
}) {
    return (
        <li className="w-[19.5rem]">
            <div className="relative">
                ...
                {false && (
                    <span className="absolute bottom-1 right-1 bg-white rounded-md px-1 text-sm">
                        {parseLength(/* item.length */ 0 * 60 * 60 * 1000)}
                    </span>
                )}
            </div>
            ...
            <div className="flex pt-3 pb-8 gap-x-3">
                {false && (
                    <img
                        src={'supposed-to-be-studio-image'}
                        className="rounded-full bg-slate-300 w-10 h-10"
                    />
                )}
                <div>
                    ...
                    {item.productionHouse ? (
                        <p className="text-sm">
                            {(item.productionHouse as string[])[0]}
                        </p>
                    ) : (
                        <></>
                    )}
                    <p className="text-sm">{`${parseViews(
                        /* item.views || */ 0
                    )} - ${
                        item.released &&
                        parsePublishedDate(new Date(item.released))
                    }`}</p>
                </div>
            </div>
        </li>

...
export default function VideoListSection({
    ...
    function moveRight() {
        ...
        if (slider) {
            ...
            if (slider.scrollLeft - prevState < 500) {
                setIsLoading(true)
                fetchMore(page)
                    .then((r: Prisma.MovieCreateInput[]) => {
    ...
    return (
        <div className="px-3 bg-gradient-to-b from-[#00000022] to-[#000000dd] bg-[#00000055] rounded-lg pb-3 mb-5">
            {name && <h1 className="py-2 text-lg">{name}</h1>}
            <div className="relative flex">
                <div
                    className="bg-[#00000055] cursor-pointer hover:bg-[#00000000] transition duration-300 absolute self-center z-30 h-3/4 -left-3 fill-[#aaaaaaaa] rounded-md flex items-center"
                    onClick={moveLeft}
                >
                ...
                <ul
                    className="gap-x-4 w-full h-full overflow-x-scroll flex no-scrollbar scroll-smooth"
                    id="slider"
                >
                ...
                <div
                    className="bg-[#00000055] cursor-pointer hover:bg-[#00000000] transition duration-300 absolute self-center z-30 h-3/4 -right-3 fill-[#aaaaaaaa] rounded-md flex items-center"
                    onClick={moveRight}
                >
                    <button className="-rotate-90">
                        <RightSign />
